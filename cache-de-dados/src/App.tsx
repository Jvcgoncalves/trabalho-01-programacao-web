...
function App() {
  ...
  async function getData() {
    try {
      if (!amiiboData) {
        ...
        localStorage.setItem("amiibo-data", JSON.stringify(response.data.amiibo));
  ...
  useEffect(() => {
    const data = localStorage.getItem("amiibo-data");

    if (data) {
      const dataFormated = JSON.parse(data);
      setDataToDisplay(dataFormated.slice(0, amountToShow))
    } else {
      getData();
    }
  ...
  useEffect(() => {
    return (() => {
      localStorage.removeItem("amiibo-data");
    })
  }, [])
  
  return (
    <div className='container-fluid bg-dark'>
      <div className='content w-50 d-flex align-items-center flex-column text-white mx-auto'>
